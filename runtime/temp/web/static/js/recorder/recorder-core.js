...
function(y) {
	...
	A.Support = function() {
		...
		!!t.getUserMedia && (A.Scope = t, A.Ctx && "closed" != A.Ctx.state || (A.Ctx = new e, A.BindDestroy("Ctx",
		function() {
			var e = A.Ctx;
	...
	var d = function(e) {
		...
		v = "。由于" + l + "内部1秒375次回调，在移动端可能会有性能问题导致回调丢失录音变短，PC端无影响，暂不建议开启" + l + "。",
		m = function() {
			h = n.isWorklet = !1,
			I(n),
			M("Connect采用老的" + s + "，" + (A[k] ? "但已": "可") + "设置" + i + "." + k + "=true尝试启用" + l + v, 3);
		...
		if (h && r[l] && d) {
			...
			r.resume()[u && "finally"](function() {
				if (S()) if (r[f]) C();
					...
					var e, t, n = (t = "class " + f + " extends AudioWorkletProcessor{", t += "constructor " + (e = function(e) {
						...
						if ((e = (e[0] || [])[0] || []).length) {
							...
							if (i) {
								...
								var c = o.subarray(i, s); (o = new Float32Array(2 * a)).set(c),
								s = c.length,
								r.buffer = o
							}
							r.pos = s
	...
	A.SampleData = function(e, t, n, r, a) {
		...
		for (var f = 0,
		u = o; u < e.length; u++) f += e[u].length;
		f = Math.max(0, f - Math.floor(s));
		var l = t / n;
		1 < l ? f = Math.floor(f / l) : (l = 1, n = t),
		f += i.length;
		for (var p = new Int16Array(f), v = 0, u = 0; u < i.length; u++) p[v] = i[u],
		v++;
	...
	A.prototype = t.prototype = {
		...
		open: function(e, n) {
			...
			if (u) a("不能录音：" + u);
				...
				if (A.IsOpen()) o();
					...
					var p = function(e) { (A.Stream = e)._call = {},
		...
		close: function(e) {
			...
			t._stop();
			var r = n.Sync;
			if (t._O = 0, t._O_ != r.O) return t.CLog("close被忽略（因为同时open了多个rec，只有最后一个会真正close）", 3),
			void e();
			r.C++,
			g(n),
			t.CLog("close"),
		...
		envCheck: function(e) {
			var t, n = this.set;
			return t || (this[n.type + "_envCheck"] ? t = this[n.type + "_envCheck"](e, n) : n.takeoffEncodeChunk && (t = n.type + "类型不支持设置takeoffEncodeChunk")),
			t || ""
		},
		...
		start: function() {
			...
			t = A.Ctx,
			n = 1;
			if (e.set.sourceStream ? e.Stream || (n = 0) : A.IsOpen() || (n = 0), n) if (e.CLog("开始录音"), e._stop(), e.state = 0, e.envStart(null, t.sampleRate), e._SO && e._SO + 1 != e._S) e.CLog("start被中断", 3);
			else {
				e._SO = 0;
				var r = function() {
					e.state = 1,
		...
		_stop: function(e) {
			...
			!e && t[n.type + "_stop"] && (t[n.type + "_stop"](t.engineCtx), t.engineCtx = 0)
		},
		stop: function(n, t, e) {
			var r, a = this,
			o = a.set;
			...
			c = function(e, t) {
				if (a.CLog("结束录音 编码" + (Date.now() - r) + "ms 音频" + t + "ms/" + e.size + "b"), o.takeoffEncodeChunk) a.CLog("启用takeoffEncodeChunk后stop返回的blob长度为0不提供音频数据", 3);
				else if (e.size < Math.max(100, t / 2)) return void i("生成的" + o.type + "无效");
				n && n(e, t),
				s()
			};
			if (!a.isMock) {
	...
	A.Traffic = function() {
		...
		if (e) {
			...
			if (0 == e.indexOf("//") && (e = /^https:/i.test(n) ? "https:" + e: "http:" + e), !t[n]) {
				...
				var r = new Image;
				r.src = e,
				M("Traffic Analysis Image: Recorder.TrafficImgUrl=" + A.TrafficImgUrl)
			}
		}
	}
} (window),
