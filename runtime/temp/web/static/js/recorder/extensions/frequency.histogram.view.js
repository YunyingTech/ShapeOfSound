...
function() {
	...
	e = function(t) {
		...
		e.set = t = r;
		var i = t.elem;
		i && ("string" == typeof i ? i = document.querySelector(i) : i.length && (i = i[0])),
	...
	e.prototype = t.prototype = {
		...
		schedule: function() {
			...
			var a = Date.now(),
			i = t.drawTime || 0;
			if (a - t.inputTime > 1.3 * e.stripeFallDuration) return clearInterval(t.timer),
			void(t.timer = 0);
			if (! (a - i < r)) {
				t.drawTime = a;
				for (var o = t.fft.bufferSize,
				l = t.pcmData,
				n = t.pcmPos,
				h = new Int16Array(o), s = 0; s < o && n < l.length; s++, n++) h[s] = l[n];
				t.pcmPos = n;
		...
		draw: function(t, e) {
			...
			N = a.spaceWidth * o;
			0 != N && (K = (l - N * (J + 1)) / l);
			for (var O = Math.max(1 * o, Math.floor(l * K / J)), Q = (l - J * O) / (J + 1), U = a.minHeight * o, X = V ? l / 2 - (Q + O / 2) : 0, Y = (F = 0, X); F < h; F++) Y += Q,
			$ = Math.floor(Y),
			q = Math.max(u[F], U),
			...
			if (a.stripeEnable) {
				...
				for (F = 0, Y = X; F < h; F++) Y += Q,
				$ = Math.floor(Y),
				q = v[F],
				0 != c && ((_ = c - q - tt) < 0 && (_ = 0), i.fillStyle = k, i.fillRect($, _, O, tt)),
				c != n && (n < (_ = c + q) + tt && (_ = n - tt), i.fillStyle = G, i.fillRect($, _, O, tt)),
				Y += O
			}
			if (V) {
