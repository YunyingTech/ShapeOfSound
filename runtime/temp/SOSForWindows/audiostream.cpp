...
void pcm_downsample(const BYTE* Buffer, const size_t BufferSize,
                    const WAVEFORMATEX* WaveFormat,BYTE & OutBuffer) {
  short sRead = 0;
  short tempSum = 0;
  int size = 0;
  int flag = 0;
  DWORD nSampleRate = WaveFormat->nSamplesPerSec;
  ...
  while (!feof(fp)) {
    ...
    if (size > 0) {
      ...
      if (flag == 3)  // 如果取了三个点
      {
        ...
        tempSum =
            tempSum /
            3;  // 求平均值，你可以根据自己需要去修改，不一定要求平均值，可以直接取一个点
...
bool startCapture(std::string path){
    qDebug() << "Thread Start...";
    ...
    */
    ...
    // 将捕获的数据保存到WAV文件
    SaveWaveData(pbyCaptureBuffer, nCurrentCaptureIndex, pwfx , path);
